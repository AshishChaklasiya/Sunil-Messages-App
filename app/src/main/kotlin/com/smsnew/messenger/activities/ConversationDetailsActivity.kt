...
import com.smsnew.messenger.adapters.ContactsAdapter
...
import com.smsnew.messenger.commonsLibCustom.helpers.KEY_PHONE
...
import com.smsnew.messenger.databinding.ActivityConversationDetailsBinding
import com.smsnew.messenger.dialogs.RenameConversationDialog
...
class ConversationDetailsActivity : SimpleActivity() {
    ...
    private fun initButton() {
        ...
        var drawableSMS = AppCompatResources.getDrawable(this, R.drawable.ic_phone_vector)
...
//        var drawableCall = AppCompatResources.getDrawable(this, R.drawable.ic_videocam_vector)
...
//        twoButton.setCompoundDrawablesWithIntrinsicBounds(null, drawableCall, null, null)
        ...
        var drawableVideoCall = AppCompatResources.getDrawable(this, R.drawable.ic_person_rounded)
        ...
        var drawableMail = AppCompatResources.getDrawable(this, R.drawable.ic_ios_share)
...
//        setupTopAppBar(binding.conversationDetailsAppbar, NavigationIcon.Arrow)
    ...
    private fun setupTextViews() {
        binding.conversationName.apply {
            ResourcesCompat.getDrawable(
                resources,
                R.drawable.ic_edit_vector, theme
            )?.apply {
    ...
    private fun updateButton() = binding.apply {
        ...
        if (participants.size == 1 && !isSpecialNumber()) {
            ...
            oneButton.setOnLongClickListener { toast(R.string.call); true; }
        }
                ...
                getContactFromAddress(address) { simpleContact ->
                    if (simpleContact != null) {
                        runOnUiThread {
                            ...
                            threeButton.setOnLongClickListener { toast(R.string.contact_details); true; }
...
//                        threeButton.isClickable = false
                        val drawable = ResourcesCompat.getDrawable(resources, R.drawable.ic_add_person_vector, theme)?.apply {
                            applyColorFilter(getProperPrimaryColor())
                        }
                        root.post {
                            threeButton.setCompoundDrawablesWithIntrinsicBounds(null, drawable, null, null)
                            threeButton.setText(R.string.add_contact)
                            topConversationDetails.conversationDetailsImage.contentDescription = getString(R.string.add_contact)
                            ...
                            threeButton.setOnLongClickListener { toast(R.string.add_contact); true; }
                        }
                    }
                }
            } else {
        ...
        val blockColor = if (isBlockNumbers) {
            primaryColor
        } else {
            red
        }
        blockButton.apply {
            ...
            val unblockText = if (participants.size == 1) R.string.unblock_number else R.string.unblock_numbers
            val blockText = if (participants.size == 1) R.string.block_number else R.string.block_numbers
            blockButton.text = if (isBlockNumbers) {
                resources.getString(unblockText)
            } else {
                resources.getString(blockText)
            }
    ...
    private fun blockNumber() {
        ...
        val numbersString = TextUtils.join(", ", numbers)
        val isBlockNumbers = isBlockNumbers()
        val baseString = if (isBlockNumbers) R.string.unblock_confirmation else R.string.block_confirmation
            ...
            ensureBackgroundThread {
                numbers.forEach {
                    if (isBlockNumbers) {
                        ...
                        binding.blockButton.setTextColor(resources.getColor(R.color.red_missed, theme))
                    } else {
                        addBlockedNumber(it)
                        val unblockText = if (participants.size == 1) R.string.unblock_number else R.string.unblock_numbers
